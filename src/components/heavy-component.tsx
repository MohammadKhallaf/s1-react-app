...
import { Container, Row, Col, Spinner, ProgressBar, Card, Button } from 'react-bootstrap';

// Generate massive data (this will eat memory)
const generateMassiveDataset = (count: number): { id: string; value: number; }[] => {
...
const heavyCalculationOnDataset = (data: { id: string; value: number; }[]): number => {
  let total = 0;
            ...
            <Card.Body>
              ...
              {isLoading ? (
                <>
                  ...
                  <ProgressBar now={progress} label={`${progress}%`} animated striped variant="danger" />
                </>
              ) : (
                <>
                  <h4 className="text-success mb-3">Done!</h4>
                  <p><strong>Calc Result:</strong> {calcResult?.toFixed(2)}</p>
                  <p><strong>Dataset Size:</strong> {dataset.length.toLocaleString()} items</p>

                  <div style={{ maxHeight: '300px', overflowY: 'auto', border: '1px solid #eee' }}>
                    <ul className="list-unstyled">
                      {dataset.slice(0, 100).map(item => (
                        <li key={item.id}>{item.id}: {item.value.toFixed(2)}</li>
                      ))}
                    </ul>
                  </div>

                  <Button variant="danger" className="mt-4" onClick={startHeavyProcess}>
                    Run Again
                  </Button>
                </>
              )}
...
export default HeavyWeightComponent;
